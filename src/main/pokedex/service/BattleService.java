package src.main.pokedex.service;
...
public class BattleService {
    ...
    public Pokemon batalhar(Pokemon p1, Pokemon p2, Scanner sc) throws InterruptedException {
        ...
        while (true) {
            turno++;
            System.out.print("\n");
            System.out.println(primeiro.getName() + " Lv." + primeiro.getLevel() + ": " + vidaP1 + " / " + primeiro.getStats().getHp());
            System.out.println(segundo.getName() + " Lv." + segundo.getLevel() + ": " + vidaP2 + " / " + segundo.getStats().getHp());
            System.out.print("\n");
            System.out.println("Golpes de " + primeiro.getName() + ":");
            i = 1;
            for (Move g : primeiro.getMoves()) {
            ...
            golpeP1 = primeiro.getMoves().get(indexGolpes);
            System.out.print("\n");
            System.out.println("Golpes de " + segundo.getName() + ":");
            i = 1;
            for (Move g : segundo.getMoves()) {
                System.out.println(i + ". " + g);
                i++;
            }
            opcaoGolpe = InputUtils.lerInt("Insira uma das opcoes acima: ", sc);
            ...
            indexGolpes = opcaoGolpe - 1;
            golpeP2 = segundo.getMoves().get(indexGolpes);
            danoP1 = (golpeP1.getCategory() == MoveCategory.PHYSICAL) ? calcularDanoFisico(primeiro, segundo, golpeP1) : calcularDanoEspecial(primeiro, segundo, golpeP1);
            ...
            danoP2 = (golpeP2.getCategory() == MoveCategory.PHYSICAL) ? calcularDanoFisico(segundo, primeiro, golpeP2) : calcularDanoEspecial(segundo, primeiro, golpeP2);
    ...
    public double calcularDanoFisico(Pokemon atacante, Pokemon defensor, Move golpe) {
        ...
        double d2 = (golpe.getDamage() * atacante.getStats().getAttack()) / defensor.getStats().getDefense();
        return ((d1 * d2) / 50) + 2;
    }
    public double calcularDanoEspecial(Pokemon atacante, Pokemon defensor, Move golpe) {
        double d1 = ((2 * atacante.getLevel()) / 5) + 2;
        double d2 = (golpe.getDamage() * atacante.getStats().getSpecialAttack()) / defensor.getStats().getSpecialDefense();
        return ((d1 * d2) / 50) + 2;
    }
    ...
    public Pokemon definirPrimeiro(Pokemon p1, Pokemon p2) {
        return (p1.getStats().getSpeed() >= p2.getStats().getSpeed()) ? p1 : p2;
    }
